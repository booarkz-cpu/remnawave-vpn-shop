...
# Current migration head: 0037_v2_5_0_tariff_constructor
...
ARTIFACT="remnawave_vpn_shop_v2_5_0_full_release.zip"
...
PY
rm -f "$ARTIFACT" "${ARTIFACT%.zip}.sha256" "${ARTIFACT%.zip}_manifest.json"
...
m=json.load(open(p)); m['sha256']=None; m['tests']=None; m['artifact']=sys.argv[2]; m['version']=sys.argv[1]; m['migration_head']='0037_v2_5_0_tariff_constructor'; m['detached_manifest']=sys.argv[2].removesuffix('.zip')+'_manifest.json'; m['signed']=False
...
PY
zip -qr "$ARTIFACT" . -x '*/.env' '.env' '*/node_modules/*' '*/__pycache__/*' '*.pyc' '.pytest_cache/*' '.git/*' '*.zip' '*.sha256' '*_manifest.json' 'release-manifest.json'
...
PY
sha256sum "$ARTIFACT" > "${ARTIFACT%.zip}.sha256"
