# Remnawave VPN Shop 3.1.1 — Установка одной командой

Версия **3.1.1** не меняет схему базы и не заменяет APK. Она сохраняет секреты staging при повторном сохранении, печатает `[CHECKOUT]` и описывает production gate. Пошаговый разбор каждого вопроса — `INSTALL_STEPS.md`. Панель — раздел 9.15 `INSTRUCTION.md`. Исправления **3.1.0** и **3.0.1** остаются. Обновление установленной копии: `sudo bash /opt/vpn-shop/scripts/update-from-github.sh`.
...
Version **3.1.1** does not change the database schema and does not replace the APKs. It keeps staging secrets on a later save, prints `[CHECKOUT]`, and documents the production gate. Every prompt is in `INSTALL_STEPS.md`. The panel order is section 9.15 of `INSTRUCTION.md`. The **3.1.0** and **3.0.1** fixes remain. Update an installed copy with `sudo bash /opt/vpn-shop/scripts/update-from-github.sh`.
