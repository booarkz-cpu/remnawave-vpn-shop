# Remnawave VPN Shop 3.1.2 — Установка одной командой

Версия **3.1.2** не меняет схему базы и не заменяет APK. Она закрывает утечки учётных данных VPN и текста исключений в ответах панели. Пошаговый разбор каждого вопроса — `INSTALL_STEPS.md`. Панель — раздел 9.16 `INSTRUCTION.md`. Сохранение секретов staging, `[CHECKOUT]` и production gate остаются от **3.1.1** (раздел 9.15). Исправления **3.1.0** и **3.0.1** остаются. Обновление установленной копии: `sudo bash /opt/vpn-shop/scripts/update-from-github.sh`.
...
Version **3.1.2** does not change the database schema and does not replace the APKs. It closes leaks of VPN credentials and exception text in panel responses. Every prompt is in `INSTALL_STEPS.md`. The panel changes are section 9.16 of `INSTRUCTION.md`. Staging-secret preservation, `[CHECKOUT]`, and the production gate remain from **3.1.1** (section 9.15). The **3.1.0** and **3.0.1** fixes remain. Update an installed copy with `sudo bash /opt/vpn-shop/scripts/update-from-github.sh`.
