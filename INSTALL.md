# Remnawave VPN Shop 3.1.4 — Установка одной командой

Версия **3.1.4** не меняет схему базы и не заменяет APK. Первый `docker compose up` больше не роняет API: миграции backend и worker сериализованы. Если контейнер всё же не стал healthy, скрипт печатает логи backend и worker. Команда `curl | bash` по-прежнему читает вопросы с терминала SSH (это исправление **3.1.3**, раздел 9.17). Пошаговый разбор каждого вопроса — `INSTALL_STEPS.md`. Запуск — раздел 9.18 `INSTRUCTION.md`. Сохранение секретов staging, `[CHECKOUT]` и production gate остаются от **3.1.1** (раздел 9.15). Обновление установленной копии: `sudo bash /opt/vpn-shop/scripts/update-from-github.sh`.
...
Version **3.1.4** does not change the database schema and does not replace the APKs. The first `docker compose up` no longer drops the API: backend and worker migrations are serialized. If a container still does not become healthy, the script prints the backend and worker logs. `curl | bash` still reads questions from the SSH terminal (the **3.1.3** fix, section 9.17). Every prompt is in `INSTALL_STEPS.md`. The boot fix is section 9.18 of `INSTRUCTION.md`. Staging-secret preservation, `[CHECKOUT]`, and the production gate remain from **3.1.1** (section 9.15). Update an installed copy with `sudo bash /opt/vpn-shop/scripts/update-from-github.sh`.
