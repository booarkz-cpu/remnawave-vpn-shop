# Remnawave VPN Shop 3.1.3 — Установка одной командой

Версия **3.1.3** не меняет схему базы и не заменяет APK. Команда `curl | bash` читает вопросы с терминала SSH. В **3.1.2** тот же запуск останавливался на первом вопросе. Пошаговый разбор каждого вопроса — `INSTALL_STEPS.md`. Панель — раздел 9.16 `INSTRUCTION.md`, установка — раздел 9.17. Сохранение секретов staging, `[CHECKOUT]` и production gate остаются от **3.1.1** (раздел 9.15). Исправления **3.1.0** и **3.0.1** остаются. Обновление установленной копии: `sudo bash /opt/vpn-shop/scripts/update-from-github.sh`.
...
Version **3.1.3** does not change the database schema and does not replace the APKs. `curl | bash` reads questions from the SSH terminal. In **3.1.2** the same command stopped on the first question. Every prompt is in `INSTALL_STEPS.md`. The panel changes are section 9.16 of `INSTRUCTION.md`, and the install fix is section 9.17. Staging-secret preservation, `[CHECKOUT]`, and the production gate remain from **3.1.1** (section 9.15). The **3.1.0** and **3.0.1** fixes remain. Update an installed copy with `sudo bash /opt/vpn-shop/scripts/update-from-github.sh`.
