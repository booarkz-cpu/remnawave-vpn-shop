# Remnawave VPN Shop 2.9.0 — Установка одной командой

Предыдущие установщики: 2.8.0 (тексты и логотип приложений), 2.7.0 (приложения Android и iOS), 2.6.0 (платформа антиабьюза и агент узла), 2.5.0 (конструктор тарифов и мониторинг узлов) и 2.4.0 (личный кабинет). Серверная схема остаётся на миграции `0038_v2_6_0_platform`. Установщик VPS сервер ставит, а APK на телефон не копирует. Android-пакеты лежат во вложениях релиза 2.9.0 и собираются скриптом `scripts/build-android-apk.sh`. iOS собирается в Xcode. Тексты и логотип задаются в админке, раздел 9.7 `INSTRUCTION.md`. Установка APK — раздел 9.8. Проверка без касс — раздел 9.4. Агент — раздел 9.5. Лицензия — `LICENSE`.
