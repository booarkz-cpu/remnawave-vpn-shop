# Remnawave VPN Shop 2.8.0 — Установка одной командой

Предыдущие установщики: 2.7.0 (приложения Android и iOS), 2.6.0 (платформа антиабьюза и агент узла), 2.5.0 (конструктор тарифов и мониторинг узлов) и 2.4.0 (личный кабинет). Серверная схема остаётся на миграции `0038_v2_6_0_platform`. Приложения установщик VPS не ставит: их собирают из `mobile/`. Тексты и логотип задаются в админке, раздел 9.7 `INSTRUCTION.md`. Проверка без касс — раздел 9.4. Агент — раздел 9.5. Лицензия — `LICENSE`.
