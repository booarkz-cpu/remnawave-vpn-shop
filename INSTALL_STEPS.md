# Пошаговая установка — Remnawave VPN Shop 3.1.4
...
На Debian или Ubuntu скрипт ставит Docker Engine и Docker Compose, копирует проект в `/opt/vpn-shop`, пишет `.env` с правами `0600`, собирает образы, поднимает PostgreSQL 16, Redis 7, API, worker, бота, админку, Mini App, личный кабинет и Caddy. Миграции Alembic доходят до `0038_v2_6_0_platform`. Версия установщика — **3.1.4**. Вопросы те же, что в **3.1.3**, **3.1.2** и **3.1.1**. Пробелы по краям ответа снимаются. `Moscow` записывается как `Europe/Moscow`.
...
В конце скрипт печатает адреса Admin, Mini App, Cabinet и API, версию `3.1.4` и почту администратора. Сгенерированный пароль печатается одной строкой `Admin password (generated)`. Сохраните его вне сервера и включите 2FA: админка → **Безопасность**.
...
Ответ `/health` содержит `"ok": true`, `"version": "3.1.4"`, `"database": true` и `"redis": true`. Предыдущий релиз отвечал `"version": "3.1.3"`. Релиз **3.1.2** отвечал `"version": "3.1.2"`. `doctor.sh` рассчитан на каталог `/opt/vpn-shop` и запущенный Docker.
...
# Step-by-step install — Remnawave VPN Shop 3.1.4
...
On Debian or Ubuntu the script installs Docker Engine and Docker Compose, copies the project to `/opt/vpn-shop`, writes `.env` as mode `0600`, builds the images, and starts PostgreSQL 16, Redis 7, the API, the worker, the bot, the admin UI, the Mini App, the user cabinet, and Caddy. Alembic migrates to `0038_v2_6_0_platform`. The installer version is **3.1.4**. The prompts are the same as in **3.1.3**, **3.1.2**, and **3.1.1**. Surrounding spaces are removed. `Moscow` is stored as `Europe/Moscow`.
...
The script prints Admin, Mini App, Cabinet, and API URLs, version `3.1.4`, and the administrator email. A generated password is the line `Admin password (generated)`. Store it off the server and turn on 2FA under **Безопасность** (Security).
...
`/health` returns `"ok": true`, `"version": "3.1.4"`, `"database": true`, and `"redis": true`. The previous release answered `"version": "3.1.3"`. Release **3.1.2** answered `"version": "3.1.2"`. `doctor.sh` expects `/opt/vpn-shop` and a running Docker engine.
